...
export async function closeDatabaseConnection(churchSlug: string): Promise<void> {
...
export function getChurchSlugFromHost(host: string): string {
  ...
  if (cleanHost.includes('.')) {
    ...
    if (parts.length >= 3 && (parts[1] === 'myapp' || parts[1] === 'churchapp')) {
      return parts[0];
    }
...
export async function initializeChurchDatabase(churchSlug: string): Promise<void> {
  ...
  try {
    ...
    console.error(`Failed to initialize database for church ${churchSlug}:`, error);
    throw error;
  }
}

// Health check for a specific church database
export async function checkDatabaseHealth(churchSlug: string): Promise<boolean> {
  try {
    const prisma = getDatabaseConnection(churchSlug);
    await prisma.$queryRaw`SELECT 1`;
    return true;
  } catch (error) {
    console.error(`Database health check failed for church ${churchSlug}:`, error);
    return false;
  }
}