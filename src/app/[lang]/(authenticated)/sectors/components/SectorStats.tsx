...
import { RiGovernmentLine, RiGitMergeLine } from 'react-icons/ri';
...
export default function SectorStats() {
  const { data: stats, isLoading, isFetching } = useSectorStats();
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Sin Célula</CardTitle>
          <FaUserXmark className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">
            {stats?.membersWithoutCell || 0}
          </div>
