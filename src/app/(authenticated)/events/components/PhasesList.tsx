...
export function PhasesList({ initialPhases }: PhasesListProps) {
  const [phases] = useState(initialPhases); // In a real app with real-time updates or revalidation, we might rely on router.refresh data passed as prop primarily.
  // Actually, since server actions revalidatePath, the parent page will pass new props. 
  // ensuring the table updates means we should just use props directly or sync state.
  // The simple way: use props directly.

  const [isEditOpen, setIsEditOpen] = useState(false);
  const [selectedPhase, setSelectedPhase] = useState<(EventPhases & { isSystem: boolean }) | undefined>(undefined);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [phaseToDelete, setPhaseToDelete] = useState<(EventPhases & { isSystem: boolean }) | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <DeleteConfirmationModal
        ...
        description={phaseToDelete?.isSystem ? "Esta es una fase del sistema y no puede ser eliminada." : undefined}
        onCancel={() => {
          setIsDeleteModalOpen(false);
          setPhaseToDelete(null);
        }}
